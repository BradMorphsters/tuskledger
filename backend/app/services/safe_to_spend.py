...
def _budget_remaining_pro_rata(
    ...
    """"What you'd normally spend before payday", pro-rated off this month's budget.
    ...
    if adherence is not None:
        ...
        days_left_in_month = days_in_month - today.day + 1  # inclusive of today
        ratio = min(days_until_paycheck / days_left_in_month, 1.0) if days_left_in_month > 0 else 1.0
        total_remaining = sum(
            max(line["limit"] - line["spent"], 0.0) for line in adherence["detail"]
        )
        return round(total_remaining * ratio, 2), "budget"

    # No budget this month: fall back to a trailing daily variable-spend
    # rate. Mirrors the personal/transfer/refund rules routers/analytics.py
    # uses elsewhere (spending = amount > 0 OR is_refund, business excluded)
    # rather than budget_health's category-keyed version, since there's no
    # per-category limit to key off of here.
    ...
    return round(daily_rate * days_until_paycheck, 2), "trailing_average"
...
def compute_safe_to_spend(db: Session, today: Optional[datetime.date] = None) -> dict:
    ...
    budget_remaining_pro_rata, budget_source = _budget_remaining_pro_rata(
        db, today, days_until_paycheck
    )
    safe_to_spend = spendable_cash - bills_due - budget_remaining_pro_rata
    ...
    if next_paycheck_source == "month_end_fallback":
        notes.append(
            "No recurring income detected; assuming your next paycheck lands "
